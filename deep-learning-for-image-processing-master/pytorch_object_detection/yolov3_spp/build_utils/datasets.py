...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
        ...
        # 如果为true(验证模式开启rect)，会使用类似原图像比例的矩形(让最长边为img_size)
        # 训练时，图像大小为img_size x img_size
        # 注意: 开启rect后，mosaic就默认关闭
        if self.rect:
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        r = self.img_size / max(h0, w0)  # resize image to img_size，并不改变原图像大小比例
...
def random_affine(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
    ...
    if n:
        ...
        # create new boxes
        # 对transform后的bbox顶点坐标进行修正(假设变换后的bbox变成了菱形，此时要修正成矩形)
        x = xy[:, [0, 2, 4, 6]]  # [n, 4]
        y = xy[:, [1, 3, 5, 7]]  # [n, 4]
